"""Processing for Query Performance experiment (Part 2).
...
class Aggregate:
    ...
    implementation: str
    k: str  # Empty string for Naive and Lookup
    n: int
    query_type: str
...
def read_and_aggregate() -> list[Aggregate]:
    ...
    grouped = (
        df.groupby(["implementation", "k", "n", "query_type"], observed=True)["time_per_query_ns"]
        .agg(["mean", "std", "count"])
        .reset_index()
    )
