...
def generate() -> None:
    ...
    for trial_idx in range(TRIALS):
        filepath = target_dir / f"input-{trial_idx + 1}.txt"
        trial_seed = ARBITRARY_SEED_VALUE + trial_idx

        tasks.append((filepath, trial_seed))

    print(f"Starting generation of {len(tasks)} files.")

    with ProcessPoolExecutor() as ex:
        futures = [ex.submit(generate_file, filepath, trial_seed) for filepath, trial_seed in tasks]
...
def generate_file(filepath: Path, trial_seed: int) -> None:
    ...
    queries = generate_queries(
        ...
        distribution=QueryDistribution.UNIFORM_IN_RANGE,
