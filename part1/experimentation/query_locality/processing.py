...
def read_and_aggregate() -> ProcessedData:
    ...
    for chunk in pd.read_csv(
        MEASUREMENTS_PATH,
        usecols=["implementation", "time (ns per query)", "alpha", "query distribution"],
        dtype={"implementation": "category", "alpha": "float64", "query distribution": "category"},
        chunksize=chunk_size
    ):
        # Aggregate each chunk
        chunk_grouped = (
            chunk.groupby(["implementation", "alpha", "query distribution"], observed=False)[
                "time (ns per query)"
            ]
            .agg(["sum", "count", lambda x: (x**2).sum()])
            .reset_index()
        )
        chunk_grouped.columns = ["implementation", "alpha", "query distribution", "sum", "count", "sum_sq"]
    ...
    grouped = (
        df.groupby(["implementation", "alpha", "query distribution"], observed=False)
    ...
    for _, row in grouped.iterrows():
        ...
        result[impl].append(
            Aggregate(
                ...
                distribution=row["query distribution"],
            )
        )
                ...
                writer.writerow(
                    {
                        ...
                        "query distribution": agg.distribution,
                    }
                )
